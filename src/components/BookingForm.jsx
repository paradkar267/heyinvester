...
export default function BookingForm({ preselectedProperty = '' }) {
  ...
  const handleSubmit = (e) => {
    ...
    const propertyName = available.find(p => p.slug === form.property)?.name || 'a property';
    const text = `Hi, I would like to book a site visit.\n\n*Name:* ${form.name}\n*Phone:* ${form.phone}\n*Property:* ${propertyName}\n*Date:* ${form.date}${form.message ? `\n*Message:* ${form.message}` : ''}`;
    const waUrl = `https://wa.me/919325650256?text=${encodeURIComponent(text)}`;
