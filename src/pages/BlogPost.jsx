  ...
  blocks.forEach((block, index) => {
    if (block.match(/^\d+\.\s+\*\*/)) {
      ...
      if (block.startsWith('### ')) {
        ...
        let remainingText = '';
        if (newlineIdx !== -1) {
            headingText = block.substring(0, newlineIdx).trim();
            remainingText = block.substring(newlineIdx).trim();
        }
        const text = headingText.replace('### ', '');
        ...
        renderedBlocks.push(
          <div key={index + '-heading'} style={{ display: 'flex', alignItems: 'center', gap: '16px', margin: 'var(--space-8) 0 var(--space-6)' }}>
            ...
            <h3 style={{ fontSize: '20px', color: 'var(--gray-900)', margin: 0, fontWeight: 700 }}>{text}</h3>
